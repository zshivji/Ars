...
def cluster_pos(pos, limit = 15):
    ...
    # only keep clusters with at least 2 genes (ArsA, ArsB; ArsB can be alone while ArsA cannot)
    cluster_no = [item for item in set(clusters) if list(clusters).count(item) >= 2]
